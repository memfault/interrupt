...
### Options
Starting from the 1.1.0 version, you can customize Emerald thanks to a few options. Now it is possible to set a custom header by setting the related option in the ``_config.yml`` file to "true". Then put into the ``header-custom.html`` file, your custom code.
...
Moreover it is now possible to select a reverse option that allows to put the navigation menu to the left side, by setting it to "true".

### Colors
In the ``base.scss`` file are set the basic colors:
