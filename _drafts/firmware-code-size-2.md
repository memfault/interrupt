---
title: "Shrink your Firmware with GCC"
description: "A walk through GCC configuration options to optimize firmware code size"
author: francois
---
...
_Like Interrupt? [Subscribe](http://eepurl.com/gpRedv) to get our latest posts straight to your mailbox_
...
## Setting the stage
In order to get measurable improvements in code size, we need a reasonably large code-base to begin with. To that end, we’ll use example code from [ChibiOS](http://chibios.org/dokuwiki/doku.php), a free and open source RTOS. Specifically, we’ll use their FatFS+USB example for STM32F1 MCUs.

You can reproduce our setup simply by cloning the project:
```
...
You would be hard pressed to chose trade-offs that work for every application: a math library may care about performance, while firmware code may care about code size. To that end, GCC bundles its optimizations into buckets which can be selected via the [Optimize Options](https://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html).
...
* `-Os` optimize for code size. This uses `O2` as the baseline, but disables some optimizations. For example, it will not [inline](https://en.wikipedia.org/wiki/Inline_expansion) code if that leads to a size increase.
...
Since the compiler operates on one file at a time, it does not have enough context to decide whether a function is dead code or not.  Consider a library which exposes a function to add numbers in an array: since the files in the library do not call that function, the compiler could think it is not used when other parts of our program rely on it.

The linker on the other hand has visibility into our whole program, so this is where dead code could be identified and removed. Unfortunately, linkers do not perform optimizations by default.
...
In some cases, you may want some code that is not explicitly called by anything else to remain in your program. One such example is interrupt handlers: while it may look to the linker like they are not called, they the hardware will jump to those addresses and expect the code to be there.

There are two ways to make sure a symbol isn’t garbage collected: with the `used` attribute, or with the `KEEP` linker script command.
...
For interrupts, the typical solution is to put your vector table in a section called `vectors` and mark it in the linker script:
...
LTO comes at the cost of compilation time. Even for simple programs the difference in build time will be noticeable, some of your colleagues may complain about it!

LTO also has a reputation for making it harder to debug. Newer versions of debuggers like GDB and LLDB do a much better job of coping with LTO however, and while there is still an impact it is not dramatic.
...
By default, `arm-none-eabi-gcc` ships with [newlib](https://sourceware.org/newlib/), a libc implementation optimized for embedded systems. While newlib is quite small, ARM recently added a slimmed down version of newlib dubbed “newlib-nano”, which does away with some C99 features and rewrites part of the library directly in thumb code to shrink its size. You can read more about newlib nano on [ARM’s website](https://community.arm.com/developer/ip-products/system/b/embedded-blog/posts/shrink-your-mcu-code-size-with-gcc-arm-embedded-4-7).

Libc selection in GCC is implemented with specs files. I won’t go into too many details, but the easiest way to think about specs files is that they are CFLAGS and LDFLAGS which are conveniently bundled into a single configuration. You can find the spec file for newlib-nano on [GitHub](https://github.com/littlekernel/newlib/blob/master/libgloss/arm/elf-nano.specs).
