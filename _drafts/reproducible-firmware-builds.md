...
If you have ever worked on a large scale embedded project before, you've probably run into situations where the binary behaves differently depending on where it was compiled. For example, maybe the binary completely fails to compile on one computer and on another it compiles but crashes on boot!
...
A build is said to be "Reproducible" when given a set of source code, the exact same binary can be generated by different people on different computers. Many open source projects (such as Debian GNU/Linux) are working towards making the entire build reproducible[^8].
...
Let's clone the interrupt repo and build the project:
...
Interesting, so the addresses are different between the builds. Are the sizes of the build the same?
...
Interesting, one build is larger than the other and it looks like most of that difference comes from the `.data` section.
...
Great! We no longer have paths that are absolute in the binary.
...
After making the changes let's recompile the project in both binaries now have the same build id!
...
Interesting so we are loading the value of 100 into register r0 instead of 1 in the other build.
