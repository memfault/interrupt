...
A third option, and a happy middle ground between the two storage options, consists of a combination of both. A better-balanced approach to log storage is to consider writing logs to RAM with periodic rotation to flush to flash or, better yet, push the logs up to the cloud. The greatest advantage of this approach is it allows you to limit flash writes and still gaining persistenet storage while not eating into too much of your RAM budget. 
...
For example, if we want to catch which processes are triggering the kernel's OOM killer, we can increment a counter: `OOMKill_<ProcessName>` metric that increments whenever a we see a log line resembling the following:
...
This now converts textual information into a quick snapshot of your system which can be scraped or forwarded to whatever time-series/observability backend you use. With logs to metrics compounded over many different events there is now a quick and easy way to capture failure rates, which becomes more valuable when devices in the field start to scale. 
