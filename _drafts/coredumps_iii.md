...
and perf traverse the stack. When unwinding a stack, there is information about
the previous frame that a debugger, profiler, or any other program needs to
know:
...
TODO: Add picture showing how CFA stores registers and is relative to current
registers
...
out the `DWARF` spec here. (TODO: Add link)
...
In our previous article we touched a bit on the `prstatus` ELF note. This note
contains the current status of the process, and there is one for each of our
threads. You probably guessed it, but this note has the state of all GP
registers at the time of crash for each thread! Also recall that we're receiving
the entire ELF coredump from `stdin` in our core handler.

TODO: When I have internet make a layout for the note here
...
libs to grab both our unwind info, as well as all the rendevous information our
debugger needs?
