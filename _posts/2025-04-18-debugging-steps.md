...
These steps are written in a way to try to get the most information about the
error as possible with as little effort as possible. Each step increases in
level of effort as you go along. The goal is that at some point throughout the
steps, you can gain enough information about the error to isolate and fix it. At
the end of the steps, a couple of case studies from my personal experience are
presented as a way of showcasing how the steps were applied to solve the issue.

While there are some "hardware" aspects to some of the bugs described, I should
note that this guide is designed for debugging "software-induced" errors. This
means that the following errors (while just as critical to fix) are out of the
...
Unless you’ve never successfully gotten your firmware to boot at all, presumably
there was a time when your firmware functioned in some capacity. Being able to
diff your codebase between "working" and "broken" states is crucial for giving
you a nudge in the right direction of what might be broken. You may not
understand precisely what is happening at this point yet, but that’s ok. Even
knowing the _general area_ the problem might be in is valuable information.

Try to find the smallest possible change you can make that induces the bad
state. Depending on when you last made a commit, this might involve making some
incremental additions between the last working commit and the current broken
...
one of my examples. Philip Johnston over at Embedded Artistry has written a
[great blog post](https://embeddedartistry.com/blog/2023/09/20/leveraging-your-toolchain-to-improve-security/)
on some of the flags he uses, as well as why they’re important.

### Step 3 - Utilize All MCU Error Reporting Capabilities

Nearly every MCU I’ve worked with has some sort of fault reporting capability. A
...
Of all the problems I’ll describe in my case studies, almost all of them
eventually presented themselves as triggering a hard fault (and thus ending up
in a hard fault handler). It is common for HAL code provided by a vendor to
provide weakly defined _default_ hard fault handlers like so:
...
what’s happened can immediately point you in the direction of what the problem
could be:
...
at. For instance, if I see a hard fault due to trying to access a bad memory
address, I know to look for some form of memory "corruption". While this can
encompass a broad range of issues, I at least now know the general problem I’m
looking for.

Having exhausted some minimal effort options, it’s now time to take a more hands
on approach to isolating the problem.

### Step 4 - Isolate the Problem Surface Area with Logging and/or Debugging

The goal of this step is to try to isolate where the problem occurs to the
smallest possible piece of code. The reason I don’t say "line of code" here is
because sometimes problems (especially related to stack/heap corruption) don’t
happen in the same place each time. Firmware is often quite dynamic, responding
to stimuli from the outside world. Here, we are trying to take away as many
changing variables as possible. Ideally, we want to be able to reproduce the
fault in the same place, and the same way each time.
...
understand what’s happening. For example, you may know that at a specific
function call in your program, a bad memory access occurs. But you still have no
idea _why_ it’s occurring. You might even call this function elsewhere and it
_doesn’t_ generate a fault!

Now that you’ve hopefully isolated more or less where the error is occurring, we
can start to dig into more subtle a nuanced potential root causes.

### Step 5 - Keep an Eye on Your Stack

I’ve created a general rule for myself: Whenever there’s a hideous, hard to
repeat, totally illogical, nonsensical issue happening in my firmware, I take a
breath and ask myself: Is there any possibility I’m overflowing my stack? The
answer to that question several times has been a resounding "yes". Catching and
debugging stack overflows is a topic that in and of itself deserves an entire
article, however, there are a couple of easy things to try that might help
...
lives in memory _especially_ if you’re using some sort of RTOS. This involves
looking into your
...
Even just knowing where your stack "starts" and "ends" can be useful when
debugging. If you’re able to pause on a breakpoint near the point of failure,
you can read the stack pointer to get a quick glance at how close to an overflow
condition you are.

In a very similar vein to the stack, there’s another troublesome piece of memory
management that could be our culprit.

### Step 6 - Keep an Eye on Your Heap

Do you use dynamic memory allocation (think malloc(), calloc(), realloc()) in
your application? If you answered no, are you sure? Generally speaking, to even
use any memory allocation functions (which are provided in the standard library)
a "system call" function `_sbrk_r` needs to be defined. For convenience, many
"code generation" style vendor tools will generate a default implementation of
`_sbrk_r` for you, as well as linker scripts and symbols that locate your stack
and heap. There are also a host of functions in the C and C++ standard libraries
that (implementation dependent) end up allocating memory under the hood
(`printf`, `sprintf`, `std::string`, `std::Vector` to name only a few). The
combination of these two leads many people (myself included at one point!) to
have dynamic memory allocation occurring in their program under the hood without
them even realizing it. When starting a new project, I like to start by defining
...
fairly specific to bare metal systems like the ones we work with. Oftentimes the
heap and stack on a bare metal system are configured like so:
...
explicitly in code, checks can and should be implemented that trigger an error
...
uncharitably be called "grasping at straws". This is where you’ve gained a lot
of confidence about bad behavior you _aren’t_ doing, and yet you’re still
getting that hard fault, nonsensical memory access, etc. This is where you start
to turn a critical gaze toward "mission-critical" peripherals that could be
...
This refers to an MCU that can map in memory external RAM or Flash ICs. A common
example many readers may have experience with would be the Raspberry Pi Pico.
This chip (the RP2040 or new RP2350) has only immutable ROM memory internally
for persistent storage, and instead executes code loaded on an external flash
IC. "Memory mapped" just means that once configured properly, the processor can
treat memory locations on these external chips as lying in a normal address
space. As will be demonstrated in one of my case studies, misconfiguring one of
these ICs can lead to VERY odd behavior. Configuration of these devices, as well
as the peripherals that enable memory mapping, should be investigated with a
fine-toothed comb.

#### DMA

DMA refers to Direct Memory Access and is a peripheral whose purpose is to
perform memory transfers in parallel while the processor does other things. The
DMA should be used _very_ carefully, as it is incredibly easy to corrupt memory
without careful usage. C and C++ have no knowledge of what the DMA is "doing" at
any given time, so extra care should be taken to check whether or not the DMA is
currently operating on a region of memory. Various naughty things that can be
done with the DMA include:
...
and data (DCache) caches. While caching can greatly speed up program execution,
it brings along with it several capital H headaches. To vastly oversimplify it,
a processor’s cache stores a copy of commonly accessed data in a special
...
(vs the actual memory location) for you. Where it starts to get complicated is
when you start bringing in peripherals like the DMA. The DMA often operates
without "letting the processor know" that the cache needs to be refreshed,
leading to fetching stale/erroneous data. If you even slightly suspect the cache
might be contributing to the issue you’re facing, turning all caching off and
seeing if the problem is resolved can be helpful. Certain processors even allow
you to manually flush the cache with a register write. So a more nuanced
approach could be adding in a call to flush the cache before a suspect
operation.

#### MPU

Some MCUs come with a "Memory Protection Unit" or MPU. This is used to designate
what is allowed to be done in specific regions of memory. For instance, one
...
Far more than a paragraph can be dedicated to all the trials and tribulations of
...
- Do you have data that can be accessed in both interrupt and non-interrupt
  contexts? If so, ensure any potential data races are protected against.
...
- Is there anywhere in your code you disable/mask interrupts? Extra care should
  be taken during these "critical sections" to keep from missing interrupts and
...
- If using an RTOS, are you using the appropriate interrupt/non-interrupt APIs
  ...
  "non-interrupt safe" APIs and you must use them correctly.

### Step 8 - MCU Errata and Application Notes

The reason this is so far down the list is that this assumes at one point you
_did_ at least glance through the errata section of your MCU’s datasheet. At
this point, we’re digging back through errata and application notes in the hopes
that our odd behavior might be attributed to a silicon bug or known issue.
...
sanity check where I put the RTT control block in a newly defined linker section
_at the end of RAM._ All of a sudden reality started to crumble. Hard faults
started appearing after a couple of logging statements. Seemingly _what_ I
logged and _how much_ affected this program crashing behavior.

#### Step 1 - See What Changed

Fortunately, quite obvious in this case. Placing the control block at the end of
RAM appears to break the code.
...
fault was occurring". This was on the Cortex M0+ based RP2040, so the hard fault
cause doesn’t get any more specific than that.

#### Step 4 - Isolate the Problem Surface Area with Logging and/or Debugging

This was pretty straightforward, the control block can be initialized without
issue, however, one or more logging statements initiate the hard fault.
...
accidentally overriding the stack. This was an incorrect assumption. I had
created a monster where entering my logging functions would push data onto the
stack, but writing bytes to my control block _would actively clobber said stack
data._ Thus oddball logging behavior where a couple of characters would transfer
...
class ThisShouldBeFun {
  ...
  void second_function() {
    // Has some sort of side effect that changes internal state of the class
  }
};
```

I had some odd behavior in my code, and after some debugging realized that even
though I was calling first_function, somehow second_function was getting
executed as well!?

#### Step 1 - See What Changed

I previously wasn’t using first_function, although it had been added to the
class a while ago so didn’t appear in any recent diffs.
...
existing issue that was brought to my attention was every so often a packet
(these boards implemented Modbus) would fail its CRC check and the command would
have to be retried. Because of this retry routine, the communication still
functioned, albeit not as fast as it could have. Eventually, and seemingly out
of the blue, the number of CRC failures in packets skyrocketed, making the
communication unusable and demanding immediate attention.

#### Step 1 - See What Changed

This was pretty maddening. None of the code changes had anything to do with code
that touched serial communication. And yet, I could revert commits and get
reliably "better" communication behavior, albeit still with occasional failures.
There also seemed to be a bizarre time component where serial communication
would seem fine at first, but after a minute or two rapidly degraded.
...
Step 7 - Vet Critical Peripherals:  
Thus started a long, and painful slog through our UART driver code. We were
using the DMA to automatically transfer received bytes into a memory buffer on
reception in the UART, and then servicing said buffer in an RTOS task whenever
...
the state of the cache. Once we fixed our cache management in regards to the
memory buffer our UART bytes were stored in, we were able to turn the cache back
on while also eliminating the mysterious bad bytes. Because the cache was often
out of date before implementing cache management, whether or not the received
bytes were correct was almost entirely random depending on other code execution
and cache hits/misses. Thus, changing unrelated bits of code changed cache
behavior and thus changed how often we got bad bytes.

### Case Study #4 - Memory Faults in the Depths of Hell

This last one is both the most painful and yet satisfying bug I’ve ever dealt
with. To set the stage, I was in the process of porting DOOM to the display
board on the TK-02 coffee machine (I currently work as the lead firmware
engineer at Terra Kaffe). This was entirely a side project for the fun and the
challenge. Porting Doom to a "bare metal" system is a little spicier than for
one with an actual OS. For instance, I replaced all allocation calls in Doom's
engine with an arena allocator that made use of an external SDRAM IC we use.
This will become relevant later. Frustratingly, things were _almost_ working. I
would occasionally get the Doom splash screen to appear on the screen, but at
...
told me I was trying to access an invalid address. The address always was of the
...
revealed we also weren’t getting close to any overflow situations regarding the
heap.
...
byte it masked happened to be the most significant byte... So, what I
essentially created was a "random memory fault generator". When the
misconfigured pin floated a little too high, it masked the MSB, mangling the
received data to be `0xFFXXXXXX`. This explained the odd engine behavior. If the
stars aligned and the SDRAM read errors only happened on non-critical textures
(and in a way that didn’t generate an MPU fault) the game would barely get to
the splash screen. In the end, it was all worth it to
