...
> [our post on the topic](/blog/a-deep-dive-into-arm-cortex-m-debug-interfaces).
...
over the
[Debug Access Port](/blog/a-deep-dive-into-arm-cortex-m-debug-interfaces#the-debug-access-port).
For the instruction in our example, this would involve updating `$r1` with the
`.word` at `$pc+4` (`0x248`) and then advancing the `$pc` by two bytes.
...
executing a breakpoint instruction will
[escalate to a HardFault](/blog/cortex-m-hardfault-debug#hardfault-status-register-hfsr---0xe000ed2c),
