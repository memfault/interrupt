...
such as the Git revision, [GNU Build ID](/blog/gnu-build-id-for-firmware), build
timestamp, and other related items. With all of this in place, both machines and
developers can quickly figure out _exactly_ which release is running on a given
device, what the source is, where it came from, and how to rebuild it if
necessary.
...
triage bugs by performing a [git bisect](/blog/git-bisect) or rebuilding old
firmwares (provided we have
...
[GNU Build ID's](/blog/gnu-build-id-for-firmware) are our final missing piece to
our versioning story. The build ID is a 160-bit SHA1 string computed over the
elf header bits and section contents in the file. Because it's computed over the
binary itself, we can be sure that every unique binary will produce a unique
build ID. Perfect!
...
The specifics on how to add a GNU Build ID are laid out in our
[previous post](/blog/gnu-build-id-for-firmware#adding-the-gnu-build-id-to-your-builds).
...
If you have your
[continuous integration](/blog/continuous-integration-for-firmware) system set
up to build your firmware, pulling the SemVer from the Git tag is a great way to
go.
...
François, in his
[Device Firmware Update Cookbook](/blog/device-firmware-update-cookbook) post,
describes how to
[include metadata](/blog/device-firmware-update-cookbook#image-metadata) in both
ELF files and into a header of loadable firmware binaries.
