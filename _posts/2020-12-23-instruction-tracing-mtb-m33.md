...
use of [watchpoints](/blog/cortex-m-watchpoints). You may recall in that post we
had to _reproduce_ the failure so we could halt the core with a watchpoint
installed in order to debug what happened.
...
![](/img/mtb-m33/da1469x-memory-map.png)

### MTB_BASE, 0xE004300C

![](/img/mtb-m33/mtb-base.png)
...
As a starting point for quick prototyping we can make use of
[GDB's Python API](/blog/automate-debugging-with-gdb-python-api) to write a
basic MTB decoder!
...
In this scenario, even if we follow the steps from our previous post on how
[to recover the callstack after a HardFault](/blog/cortex-m-hardfault-debug#registers-prior-to-exception),
there isn't a lot of context revealed.
